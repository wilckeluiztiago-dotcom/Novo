...
class GeradorDados:
    ...
    def exportar_json(
        ...
        print(f"Dados exportados para: {caminho}")
    def gerar_dados_brasil_simulados(
        self,
        anos: int = 10,
        freq: str = 'M'
    ) -> pd.DataFrame:
        """
        Gera dados macroeconômicos simulados com características do Brasil.
        
        Simula:
        - Taxa de Desemprego (PNADC)
        - Inflação (IPCA)
        - Taxa de Juros (Selic)
        - PIB (Variação trimestral interpolada)
        - Rendimento Médio Real
        
        Args:
            anos: Número de anos a simular
            freq: Frequência ('M' para mensal)
            
        Returns:
            DataFrame com as séries simuladas
        """
        if freq != 'M':
            raise NotImplementedError("Apenas frequência mensal ('M') suportada por enquanto.")
            
        n_meses = anos * 12
        datas = pd.date_range(end=datetime.now(), periods=n_meses, freq='M')
        
        # --- 1. Taxa de Desemprego (Baseada na PNADC) ---
        # Média ~9-10%, Sazonalidade forte (piora no início do ano), Tendência cíclica
        tendencia_ciclica = 0.02 * np.sin(np.linspace(0, 4*np.pi, n_meses)) # Ciclos econômicos
        sazonalidade_desemp = 0.005 * np.sin(2 * np.pi * np.arange(n_meses) / 12)
        ruido_desemp = self.rng.normal(0, 0.003, n_meses)
        
        # Choque COVID (aprox. 2/3 do caminho)
        choque_covid = np.zeros(n_meses)
        idx_covid = int(n_meses * 0.8) # Assumindo dados recentes
        if idx_covid < n_meses:
            choque_covid[idx_covid:] = 0.04 * np.exp(-np.arange(n_meses - idx_covid) / 12)
            
        desemprego = 0.09 + tendencia_ciclica + sazonalidade_desemp + ruido_desemp + choque_covid
        desemprego = np.clip(desemprego, 0.04, 0.15)
        
        # --- 2. Inflação (IPCA) ---
        # Correlação inversa com desemprego (Phillips) + Choques de oferta
        # Média ~4-5% a.a. -> ~0.3-0.4% a.m.
        inflacao_base = 0.004 
        # Phillips: Desemprego alto -> Inflação cai (com defasagem)
        efeito_phillips = -0.1 * (desemprego - 0.09) 
        choques_oferta = self.rng.normal(0, 0.002, n_meses)
        
        # Choque inflacionário recente
        if idx_covid < n_meses:
             choques_oferta[idx_covid:idx_covid+24] += 0.005 # Inflação pós-pandemia
             
        inflacao = inflacao_base + efeito_phillips + choques_oferta
        
        # --- 3. Taxa Selic (Juros) ---
        # Reage à inflação (Regra de Taylor simplificada)
        selic = np.zeros(n_meses)
        selic[0] = 0.10 # 10% a.a. inicial
        
        for t in range(1, n_meses):
            # Meta de inflação ~3.5% a.a. -> 0.0029 a.m.
            gap_inflacao = inflacao[t-1] - 0.0029
            # Ajuste gradual (suavização)
            target_selic = 0.04 + 1.5 * (gap_inflacao * 12) # Regra de Taylor anualizada
            target_selic = max(0.02, target_selic) # Zero lower bound (ou quase)
            
            selic[t] = selic[t-1] + 0.1 * (target_selic - selic[t-1]) + self.rng.normal(0, 0.001)
            
        # --- 4. PIB (Crescimento anualizado) ---
        # Lei de Okun: Crescimento alto -> Desemprego cai
        # Inverso: Desemprego caindo -> PIB crescendo
        delta_desemp = np.diff(desemprego, prepend=desemprego[0])
        pib_crescimento = 0.02 - 2.0 * delta_desemp + self.rng.normal(0, 0.01, n_meses)
        
        # --- 5. Rendimento Médio Real ---
        # Cresce com produtividade, cai com desemprego alto
        rendimento = np.zeros(n_meses)
        rendimento[0] = 2500 # R$
        
        for t in range(1, n_meses):
            crescimento_tendencia = 0.001 # Produtividade
            pressao_mercado = -0.5 * (desemprego[t-1] - 0.09) # Curva de salários
            
            var_pct = crescimento_tendencia + pressao_mercado + self.rng.normal(0, 0.005)
            rendimento[t] = rendimento[t-1] * (1 + var_pct)
